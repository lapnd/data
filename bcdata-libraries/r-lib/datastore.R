...
resizePlot = function (snapshot, w, h) 
{
    if (!file.exists(snapshot)) {
        print(paste("Can't find snapshot file ", snapshot, sep = ""))
        return - 1
    }
    base::load(snapshot)
    filename = snapshot
    filename = substr(filename, 1, regexpr(".snapshot", filename, 
        fixed = T)[1] - 1)
    filename = paste(filename, ".png", sep = "")
    if (is.null(data) || nchar(data) == 0) {
        print(paste("Snapshot data is empty, snapshot file ", 
            snapshot, sep = ""))
        return - 1
    }
    png(filename = filename, width = w, height = h)
    # the ggplot2 needs to load here in case of ggplot plot? So we should check the type of plot!?
    library(ggplot2)
    replayPlot(data)
    dev.off()
}
