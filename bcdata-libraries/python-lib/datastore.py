...
def store(data, dataset="", description=""):
  ...
  create_str = "CREATE TABLE IF NOT EXISTS `" + dataset + "` ("
  li = iter(fields)
  field = next(li)
  while True:
    try:
  ...
  schema_str = (create_str[:13] + create_str[27:])
  ...
  # drop old table
  drop_str = "DROP TABLE IF EXISTS `" + dataset + "`"
    ...
    try:
      ...
      while True:
        try:
          ...
          #else:
          if field in item:
            if (type(item[field]) is bool):
              row.append(1 if item[field] else 0)
            else:
              try:
...
def load(identifier, start=0, maximum=18446744073709551615):
  if not _initialized:
    return
  ...
  if start < 0 or maximum < 0:
    ...
    return
  query = "SELECT * FROM `" + identifier + "` LIMIT " + str(start) + "," + str(maximum)
...
def main():
    ...
    schema = "CREATE TABLE IF NOT EXISTS `demo.yahoo_sport_2` (`FumL` text,`Y/A` text,`Rush` text,`Avg` text,`Name` text,`G` text,`Y/G` text,`Comp` text,`QBRat` text,`Att` text,`Sack` text,`Int` text,`YdsL` text,`Pct` text,`Team` text,`TD` text,`Fum` text,`Yds` text)"
    conn = mysqldb.connect (_host, _user, _passwd, _db)
    cursor = conn.cursor()
